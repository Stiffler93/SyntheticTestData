...
OUTPUT_DIR = os.environ.get("SYNTHPOSE_OUTPUT_DIR") or "ownsamples/"
COUNT      = int(os.environ.get("SYNTHPOSE_COUNT")  or 15)
...
def random_skeleton(model):
    for bone in model.pose.bones:
        if bone.name.split(".")[0] in ["Upperarm", "Lowerarm", "Head", "Neck"]:
...
def set_mode(mode):
    ...
    if mode == "depth":
        ...
        bpy.data.worlds[0].mist_settings.intensity = 0.9
    elif mode == "parts":
